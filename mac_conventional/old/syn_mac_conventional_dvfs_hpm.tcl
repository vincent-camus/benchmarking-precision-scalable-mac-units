...
if {[info exists AUTO]} {
    ...
    # Design name
    set MAPPING      clk:$CLK_8B-$CLK_4B-${CLK_2B}_asym-hpm
    ...
    set DB_1p0       /scratch/camus/common/dkits/tcbn28hpmbwp35tt1v25c_ccs.lib
    set DB_0p8       /scratch/camus/common/dkits/tcbn28hpmbwp35tt0p8v25c_ccs.lib
    set DB_0p9       /scratch/camus/common/dkits/tcbn28hpmbwp35tt0p9v25c_ccs.lib
}

################# LIBRARY #################

set_attribute library $DB_1p0
...
set_attribute library "{ $DB_0p8 $DB_0p9 }"

for {set VOLTAGE 0.80} {$VOLTAGE < 0.90} {set VOLTAGE [format "%.3f" [expr {$VOLTAGE + 0.05}]]} {
...
set_attribute library "{ $DB_0p9 $DB_1p0 }"

for {set VOLTAGE 0.90} {$VOLTAGE <= 1.00} {set VOLTAGE [format "%.3f" [expr {$VOLTAGE + 0.05}]]} {
